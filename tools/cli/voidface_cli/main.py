...
from voidface_cli.common import ALLOWED_RESTORERS as _ALLOWED_RESTORERS  # noqa: E402
...
def _cmd_verify(args: argparse.Namespace) -> int:
    ...
    if not checksums_path.exists():
        print(f"error: {checksums_path} not found (not a voidface bundle?)",
              file=sys.stderr)
        return 2
    ...
    for line in text.splitlines():
        ...
        if actual_sha.lower() != expected_sha.lower():
            mismatches.append(
                f"  MISMATCH {file_name}  expected {expected_sha[:16]}...  got {actual_sha[:16]}..."
            )
        else:
...
def _cmd_package(args: argparse.Namespace) -> int:
    ...
    # Compute SHA-256 for every produced FILE artifact (skip
    # directories like .mlpackage).
    ...
    (args.output_dir / "CHECKSUMS.sha256").write_text(
        "\n".join(f"{sha}  {args.output_dir.name}/{Path(artifacts[name]).name}"
